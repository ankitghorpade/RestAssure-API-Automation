...
public class GetProfileReport {
    ...
    public GetProfileReport() throws FileNotFoundException {
    }

    @Test(dataProvider = "theTestData")
    void getProfileReport(String body, String statuscodes, String urlParam, String description, String expectedResult) {
        String nameofCurrMethod = new Throwable().getStackTrace()[0].getMethodName();// used to search the string in response
...
//        response.then().time(Matchers.lessThanOrEqualTo(5000L));
        ...
        if (response.asString().contains(nameofCurrMethod.toLowerCase())) {
            ...
            JsonArray deviceArray = jsonObject1.getAsJsonArray("device");
            JsonObject deviceObject = deviceArray.get(0).getAsJsonObject();
            String nameValue = deviceObject.get("name").getAsString();
            System.out.println(nameValue);

            response.then().assertThat().body(matchesJsonSchema(new File(config.getProfileReportSchemaJson())));
    ...
    public Object[][] excelGetProfileReport() throws IOException {
