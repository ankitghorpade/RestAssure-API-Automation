...
public class UpdateDeviceAlias {
    ...
    public UpdateDeviceAlias() throws FileNotFoundException {
    }
    ...
    public void setUp() throws IOException {
        ...
        deleteDevice.setUp();
        data.setRequestOrgName(deleteDevice.getData().getRequestOrgName());
        data.setRequestAccName(deleteDevice.getData().getRequestAccName());
        data.setRequestHostName(deleteDevice.getData().getRequestHostName());

    }

    @Test(dataProvider = "theTestData")
    public void updateDeviceAlias(String body, String statuscodes, String urlParam, String description, String expectedResult) {
        System.out.println("\n###############" + description + "###############\n");
    ...
    @AfterMethod
    public void delDevice() throws IOException {
        if (String.valueOf(response.statusCode()).equals(config.statusCode200())) {

            if ((data.getStatusMessage().equals(config.statusSuccess().toUpperCase()))) {

                RemoveDeviceAlias removeDeviceAlias = new RemoveDeviceAlias();

                removeDeviceAlias.removeDeviceAlias(config.deldevALIAS().replace("INJECT_ALIAS", data.getRequestAliasName()),
                        config.statusCode200(), data.getRequestAccName(), "REMOVE DEVICE ALIAS", config.statusSuccess().toUpperCase());
                System.out.println("\n*********************************************\n");
            }
        }
        else {
    ...
    public Object[][]  excelUpdateDeviceAlias() throws IOException {
