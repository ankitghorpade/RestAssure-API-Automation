...
public class AddDevice {
    ...
    public AddDevice() throws FileNotFoundException {
    }
    ...
    public void addDevice(String body, String statuscodes, String urlParam, String description, String expectedResult) throws IOException {
        System.out.println("\n###############" + description + "###############\n");
        ...
        data.setStatusMessage(jsonObject1.getAsJsonObject("response").getAsJsonObject("results")
                .getAsJsonArray("result").get(0).getAsJsonObject().get("status").getAsString());
    ...
    //Deleting the device if add device is success
    @AfterMethod
    public void delDevice() throws IOException {
        ...
        if (String.valueOf(response.statusCode()).equals(config.statusCode200())) {
            ...
            if ((data.getStatusMessage().equals(config.statusSuccess().toUpperCase()))) {
                ...
                deleteDevice.deleteDevice(config.delDEVICE().replace("INJECT_HOSTNAME", data.getResponseHostName()),
                        config.statusCode200(), data.getRequestAccName(), "DELETE DEVICE", config.statusSuccess().toUpperCase());
                System.out.println("\n*********************************************\n");
            }
        }
        else {
    ...
    public Object[][]  excelAddDevice() throws IOException {
