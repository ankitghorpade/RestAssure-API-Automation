...
import com.secpod.testCases.accountManagement.AddAccount;
...
public class UpdateProvisionBenchmark {
    ...
    public UpdateProvisionBenchmark() throws FileNotFoundException {
    }

    @BeforeClass
    void setUp() throws FileNotFoundException {
        provisionBenchmark.setUp();
        provisionBenchmark.provisionBenchmark(config.provisionBenchmark(),
                config.statusCode200(), "provision benchmark", config.statusSuccess().toUpperCase());
        data.setResponseAccName(provisionBenchmark.getData().getRequestAccName());
        ...
        System.out.println("Print Benchmark name:::" + data.getRequestBenchmarkName());

        AddAccount addAccount = new AddAccount();
        addAccount.addAccount(config.addACC().replace("INJECT_ORGNAME", data.getRequestOrgName())
                .replace("INJECT_ACCNAME", "Account2"), config.statusCode200(), "CREATEACC", config.statusSuccess());
        data.setRequestAccName(addAccount.getData().getRequestAccName());
    }

    @Test(dataProvider = "theTestData")
    public void updateProvisionBenchmark(String body, String statuscodes, String description, String expectedResult) {
        ...
        if ((body.contains("INJECT_ORGNAME")) || (body.contains("INJECT_BENCHMARKNAME"))) {
            body = body.replace("INJECT_ORGNAME", data.getRequestOrgName())
                    ...
                    .replace("INJECT_TARGET_ACCOUNT_NAME", data.getRequestAccName());
        }
...
//            response.then().assertThat().body(matchesJsonSchema(new File(config.jsonSchemaValidation11())));
            ...
            Assert.assertEquals(data.getResponseReasonMessage(), expectedResult
                    .replace("INJECT_TARGET_ACCOUNT_NAME", data.getResponseAccName()));
        } else {
    ...
    public Object[][] excelUpdateProvisionBenchmark() throws IOException {
